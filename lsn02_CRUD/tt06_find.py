...
ppt(list(users.find({"title": {"$in": ["HR", "Software Engineer"]}})))

fmter.tpl._straightline("""users.find({"title": {"$nin": ["HR", "Software Engineer"]}})""", 150)
ppt(list(users.find({"title": {"$nin": ["HR", "Software Engineer"]}})))

# $and, $or, $not, $nor
fmter.tpl._straightline("""users.find({"$and": [{"age": {"$gt": 20}}, {"enroll_date": {"$lt": datetime(2013, 1, 1)}}]})""", 150)
ppt(list(users.find({"$and": [{"age": {"$gt": 20}}, {"enroll_date": {"$lt": datetime(2013, 1, 1)}}]})))